...
# Script of the subsampling selection
...
selectedSamples = read.csv("Final_samples_selection.csv", head=T); indices = c(); locations1 = c(); locations2 = c()
for (i in 1:dim(allSamples)[1])
	{
		locations1 = c(locations1, paste0(gsub(" ","",allSamples[i,"city"]),"_",round(as.numeric(allSamples[i,"lon"]),4),"_",round(as.numeric(allSamples[i,"lat"]),4)))
	}
for (i in 1:dim(selectedSamples)[1])
	{
		locations2 = c(locations2, paste0(gsub(" ","",selectedSamples[i,"city"]),"_",round(selectedSamples[i,"lon"],4),"_",round(selectedSamples[i,"lat"],4)))
	}
finalSamples = which(locations1%in%locations2); DMAX = 41846453
...
points1 = cbind(as.numeric(allSamples[indices,"lon"]),as.numeric(allSamples[indices,"lat"]))
points(points1, pch=16, col="#7F7F7F30", cex=0.5, lwd=0); points(points1, pch=1, col="#7F7F7F", cex=0.5, lwd=0.4)
points2 = cbind(as.numeric(allSamples[finalSamples,"lon"]),as.numeric(allSamples[finalSamples,"lat"]))
points(points2, pch=16, col="#8151A130", cex=0.5, lwd=0); points(points2, pch=1, col="#8151A1", cex=0.5, lwd=0.4)
